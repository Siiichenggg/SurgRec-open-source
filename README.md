...
This repository is the public release for **SurgRec**: downstream fine-tuning code, the 16-dataset benchmark with consistent splits, and VLM baseline evaluation. It reproduces the paper's downstream fine-tuning results for the released backbones and the publicly obtainable general-domain baselines (DINOv3, VideoMAE, JEPA). Two columns of Table 2 — the SR-MAE (w/o bal.) ablation and DINOv3-SurgeNetXL — depend on checkpoints that are not released.
...
datasets and ties for best on 3 more (M2CAI16 with SurgRec-JEPA,
SurgicalActions160 with the unbalanced ablation, and cat-21 where every method
scores 15.69). JEPA leads on cataract-1k and SurgRec-JEPA on Cholec80. On the
mean, SurgRec-MAE is 9.4 points above the strongest other column — the
SR-MAE (w/o bal.) ablation, 35.92 — and 11.3 points above the strongest
third-party baseline, JEPA at 34.02.

Every method scores exactly 15.69 on cat-21, consistent with all of them
collapsing to a single class; that row carries no signal either way. See
[docs/benchmark.md](docs/benchmark.md) to reproduce these runs.
...
- The pretraining and data-curation pipeline, and with it the SR-MAE (w/o bal.) ablation and DINOv3-SurgeNetXL checkpoints.
