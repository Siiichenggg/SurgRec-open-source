...
from surgrec_video.augment.rand_augment import rand_augment_transform
from surgrec_video.augment.random_erasing import RandomErasing
...
# Video-specific functional transforms
class VideoFunctional:
    @staticmethod
    def resize_clip(clip, size, interpolation='bilinear'):
        """Resize a video clip to given size"""
        ...
        for frame in clip:
            ...
            if interpolation == 'bilinear':
                resized_frame = F.resize(frame, size, interpolation=Image.BILINEAR)
            elif interpolation == 'bicubic':
                resized_frame = F.resize(frame, size, interpolation=Image.BICUBIC)
            elif interpolation == 'nearest':
                resized_frame = F.resize(frame, size, interpolation=Image.NEAREST)
            else:
                resized_frame = F.resize(frame, size, interpolation=Image.BILINEAR)
            resized_clip.append(resized_frame)
        return resized_clip
    
    @staticmethod
    def crop_clip(clip, top, left, height, width):
        """Crop a video clip at specified location"""
        cropped_clip = []
        for frame in clip:
            # Handle different input types
            if isinstance(frame, np.ndarray):
            ...
            cropped_clip.append(cropped_frame)
        return cropped_clip
    
    @staticmethod
    def normalize(clip, mean, std):
        """Normalize a video clip with mean and std"""
        # Handle different input formats
        if isinstance(clip, torch.Tensor):
            # Check tensor dimensions to handle different formats
            if len(clip.shape) == 3:
                # Format: (C*T, H, W) - stacked video frames
                # This is the format from VideoMAE's Stack + ToTorchFormatTensor
                num_channels = clip.size(0)
                num_rgb_channels = len(mean)
                
                if num_channels % num_rgb_channels == 0:
                    # Create repeated mean and std for all frames
                    num_frames = num_channels // num_rgb_channels
                    rep_mean = torch.tensor(mean * num_frames, device=clip.device, dtype=clip.dtype)
                    rep_std = torch.tensor(std * num_frames, device=clip.device, dtype=clip.dtype)
                    
                    # Normalize each channel
                    for i in range(num_channels):
                        clip[i].sub_(rep_mean[i]).div_(rep_std[i])
                    return clip
                else:
                    # Fallback to standard normalization
                    return F.normalize(clip, mean, std)
            elif len(clip.shape) == 4:
                # Format: (C, T, H, W) - volume format from ClipToTensor
                # or (T, C, H, W) - separate frames
                if clip.size(0) == len(mean):
                    # Format: (C, T, H, W) - channel first, time second
                    mean_tensor = torch.tensor(mean, device=clip.device, dtype=clip.dtype)
                    std_tensor = torch.tensor(std, device=clip.device, dtype=clip.dtype)
                    # Reshape to match tensor dimensions: (C, 1, 1, 1)
                    mean_tensor = mean_tensor.view(-1, 1, 1, 1)
                    std_tensor = std_tensor.view(-1, 1, 1, 1)
                    return (clip - mean_tensor) / std_tensor
                elif clip.size(1) == len(mean):
                    # Format: (T, C, H, W) - time first, channel second
                    return F.normalize(clip, mean, std)
                else:
                    # Fallback to standard normalization
                    return F.normalize(clip, mean, std)
            else:
                # Other formats, use standard normalization
                return F.normalize(clip, mean, std)
        elif isinstance(clip, list):
            # If input is a list of frames, process each frame
            normalized_clip = []
            for frame in clip:
                ...
                normalized_frame = F.normalize(frame, mean, std)
                normalized_clip.append(normalized_frame)
            return normalized_clip
        else:
            # Single frame input
            if isinstance(clip, (Image.Image, np.ndarray)):
                if isinstance(clip, np.ndarray):
                    if clip.dtype != np.uint8:
                        clip = (clip * 255).astype(np.uint8)
                    clip = Image.fromarray(clip)
                clip = F.to_tensor(clip)
            return F.normalize(clip, mean, std)
...
def create_random_augment(
    ...
    if not auto_augment or auto_augment in {"none", "original"}:
