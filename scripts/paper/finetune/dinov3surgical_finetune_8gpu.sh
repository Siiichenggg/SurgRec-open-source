...
# 8x RTX 3090 fine-tuning script for VideoMAE with DINOv3 ViT-L/16
OUTPUT_DIR="${OUTPUT_ROOT}/dinov3surgical_ft"
DATA_PATH="${DATA_ROOT}"
EVAL_DATA_PATH="${TEST_ROOT}"
MODEL_PATH="${CKPT_ROOT}/DINOv3_ViTl16_size336_SurgeNetXL.pth"
...
# NOTE: no val/test csv in DATA_PATH -> disable eval during finetuning

cd "${PROJECT_ROOT}"

LOG_FILE="${LOG_DIR}/dinov3surgical_finetune_8gpu_$(date +%Y%m%d_%H%M%S).log"
