...
Model weights are not stored in Git. **The two SurgRec backbones are coming
soon** — they will be released on the Hugging Face Hub, and the download command
will be added here on release.
...
| --- | --- | --- | --- | --- | --- |
| SurgRec-MAE | ViT-B | Ours, masked video reconstruction | `checkpoints/surgrec_mae.pth` | 1.1 GB | Coming soon. tube mask 0.9, decoder depth 4, epoch 149. Load with `model_key=model`. |
| SurgRec-JEPA | ViT-L | Ours, latent predictive pretraining | `checkpoints/surgrec_jepa.pt` | 4.8 GB | Coming soon. epoch 131. Load with `model_key=encoder`. |
